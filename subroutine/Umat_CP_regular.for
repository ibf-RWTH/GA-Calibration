...
!------------------------------------------------------------------------------
          ...
          character(len=256) :: stopped_file_name
          character(len=256) :: stopped_file_path
          real(8) :: time(2)
          real(8) :: dtime
          LOGICAL :: file_exists
...
!---------Get current directory and jobname
          !if (irank == 0) write(*,*) 'debug_line_1'
          call getjobname(ctrl_jobname, lctrl_jobname)
          !if (irank == 0) write(*,*) 'debug_line_2'
          call getoutdir(ctrl_jobdir, lctrl_jobdir)
          !if (irank == 0) write(*,*) 'debug_line_3'
...
!             stopped_file_name = "/stopped.txt"
!             write(stopped_file_path, *) TRIM(ctrl_jobdir), TRIM(stopped_file_name)
!             open(105,file= stopped_file_path ,position='append')
!             write(105,*) ctrl_jobname
!             close(105)
          endif
          return
